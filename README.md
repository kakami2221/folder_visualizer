...
| 可視化 | ローカル配信の Plotly.js。可視化ライブラリ用CDNは使わない |
| 広告 | 404 / 500を除く通常ページでGoogle AdSense Auto adsを読み込む |
...
ブラウザのフォルダ選択 API が返す相対パスだけを扱うため、OS 上の絶対パスは取得しません。アプリ本体には、ファイル名、相対パス、検索条件、IndexedDB の内容、出力レポートを Flask へアップロードする処理はありません。通常ページでは別途Google AdSenseを読み込むため、広告コードが動作するページに解析結果が表示される点には注意してください。
...
- 可視化ライブラリ用CDN、外部フォント、アクセス解析、決済、ライセンス確認サービスは利用しません。
- 404 / 500を除く通常ページではGoogle AdSenseを利用します。広告配信に伴う通信と識別子の扱いはプライバシーページに記載します。
...
Google AdSense以外の外部サービスへの通信を追加する場合も、プライバシー前提が変わるため、UI、CSP、ログ、ドキュメントを同時に見直してください。
